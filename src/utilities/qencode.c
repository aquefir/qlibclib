 ...
 * @param count     if count is not NULL, a number of parsed entries are
 * stored.
 *
 ...
 *  qlisttbl_t *tbl = qparse_queries(NULL, req->pszQueryString, '=', '&',
...
char* qbase64_encode( const void* bin, size_t size )
{
	     ...
	     pBinPt++, nOffset++ )
	{
		...
		*pszB64Pt++ = B64CHARTBL[(
		   ( ( szIn[0] & 0x03 ) << 4 ) | ( ( szIn[1] & 0xF0 ) >> 4 ) )];
		*pszB64Pt++ = ( nIdxOfThree >= 1 )
		   ? B64CHARTBL[(
		        ( ( szIn[1] & 0x0F ) << 2 ) | ( ( szIn[2] & 0xC0 ) >> 6 ) )]
		   : '=';
		*pszB64Pt++ =
		   ( nIdxOfThree >= 2 ) ? B64CHARTBL[( szIn[2] & 0x3F )] : '=';
